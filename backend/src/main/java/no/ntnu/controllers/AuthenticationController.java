...
public class AuthenticationController {
     ...
     */
    @Operation(summary = "Setup 2FA", description = "Generates a 2FA secret key and returns a QR code URL for the user.")
    ...
    @Operation(summary = "Verify 2FA Setup", responses = {
    ...
    public ResponseEntity<?> verifyTwoFactorSetup(@RequestBody TwoFactorDTO twoFactorDTO, Principal principal) {
        try {
            String secretKey = userService.get2FASecretKey(principal.getName());
            boolean isTokenValid = userService.verify2FAToken(twoFactorDTO.getToken(), secretKey);

            if (isTokenValid) {
                logger.info("2FA token verified successfully for user: {}", principal.getName());
            ...
            logger.error("Error during 2FA verification for user: {}", principal.getName(), e);
    ...
    @Operation(summary = "Validate 2FA Token", responses = {
    ...
    public ResponseEntity<?> validateTwoFactorToken(@RequestBody TwoFactorDTO twoFactorDTO, Principal principal) {
        try {
            String secretKey = userService.get2FASecretKey(principal.getName());
            boolean isTokenValid = userService.verify2FAToken(twoFactorDTO.getToken(), secretKey);

            if (isTokenValid) {
                logger.info("2FA token validation successful for user: {}", principal.getName());
            ...
            logger.error("Error during 2FA token validation for user: {}", principal.getName(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred.");
        }
    }
}